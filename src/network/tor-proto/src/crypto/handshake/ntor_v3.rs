...
mod test {
    ...
    fn test_ntor3_roundtrip() {
        // Note: the outer `rng` is unused in the original code, keeping it for now.
        let mut rng = rand::rng();
        let mut test_rng = rand::rngs::StdRng::seed_from_u64(0);
        let relay_private = NtorV3SecretKey::generate_for_test(&mut test_rng);
    ...
    fn test_ntor3_roundtrip_highlevel() {
        let mut rng = rand::rng();
        let relay_private = NtorV3SecretKey::generate_for_test(&mut rand::rngs::StdRng::seed_from_u64(0));
